...
$pendapatan_bulan_ini = Jual::whereRaw("status_jual='TIBA' AND waktu_pesan>=? AND 
waktu_pesan<?", [Carbon::today()->firstOfMonth(), Carbon::today()
->firstOfMonth()->addMonths(1)])->sum('total_harga');
$pendapatan_minggu_terakhir = Jual::whereRaw("status_jual='TIBA' AND waktu_pesan>=? AND 
waktu_pesan<?", [Carbon::today()->subDays(6), Carbon::today()->addDays(1)])->sum('total_harga');

return view('resto.home.index', compact('juals','status_jual','arr_status_jual', 
'jual', 'rating_50', 'rating_semua','order_minggu_terakhir','order_bulan_ini','pendapatan_bulan_ini','pendapatan_minggu_terakhir'));
 }
